...
import { IngestionService, EmbeddingsService, SearchIndexService } from '@ekg/worker';
...
async function main(): Promise<void> {
  ...
  const neo4jClient = new Neo4jClient({
    uri: env.neo4jUri, user: env.neo4jUser, password: env.neo4jPassword,
  });
  try { await neo4jClient.verifyConnectivity(); }
  catch { log.warn('Neo4j not available — webhooks will fail until connected'); }
  ...
  const embeddingsService = new EmbeddingsService({
    enabled: (process.env['EKG_EMBEDDINGS_ENABLED'] ?? 'false').toLowerCase() === 'true',
    dbPath: join(env.dataDir, 'ekg-embeddings.db'),
  });
  const searchIndexService = new SearchIndexService({ dbPath: join(env.dataDir, 'ekg-search.db') });
  const unresolvedHttpRepo = new UnresolvedHttpRepository(sqliteRepo.getConnection());

  const ingestionService = new IngestionService(
    env.dataDir, neo4jClient, sqliteRepo, embeddingsService, searchIndexService, unresolvedHttpRepo,
  );
  try { await ingestionService.initGraph(); }
  catch { log.warn('Could not initialise graph indexes — Neo4j may not be running'); }
  ...
  await new Promise<void>((res) => { server.listen(webhookEnv.port, () => res()); });
  log.info({
    port: webhookEnv.port,
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    catch (e) { log.warn({ err: e instanceof Error ? e.message : String(e) }, 'queue drain failed'); }
