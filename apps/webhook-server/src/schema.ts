 ...
 */
export function shouldSkipPush(evt: GitlabPushEvent): { skip: true; reason: string } | { skip: false } {
  ...
  if (!evt.ref.startsWith('refs/heads/')) return { skip: true, reason: `non-branch ref ${evt.ref}` };
  return { skip: false };
}
 ...
 */
export function matchesAllowList(pathWithNamespace: string, patterns: readonly string[]): boolean {
