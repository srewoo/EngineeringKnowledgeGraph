...
export class IngestQueue {
  ...
  enqueue(req: IngestJobRequest): EnqueueResult {
    if (this.active.has(req.repoUrl)) {
      this.logger.info({ repoUrl: req.repoUrl }, 'webhook duplicate; ignoring (per-repo lock)');
      return { accepted: false, reason: 'duplicate-repo', queueDepth: this.pending.length, inFlight: this.inFlightCount };
    }
    this.active.add(req.repoUrl);
    this.pending.push({ req });
    this.logger.info({
      repoUrl: req.repoUrl, sha: req.commitSha, branch: req.branch,
      queueDepth: this.pending.length, inFlight: this.inFlightCount,
    }, 'webhook job enqueued');
    this.pump();
    return { accepted: true, queueDepth: this.pending.length, inFlight: this.inFlightCount };
  }
  ...
  private async runOne(req: IngestJobRequest): Promise<void> {
    ...
    try {
      ...
      this.logger.info({ repoUrl: req.repoUrl, durationMs: Date.now() - t0 }, 'webhook job completed');
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      this.logger.error({ repoUrl: req.repoUrl, err: msg, durationMs: Date.now() - t0 }, 'webhook job failed');
    } finally {
      this.inFlightCount -= 1;
  ...
  inFlight(): number { return this.inFlightCount; }
